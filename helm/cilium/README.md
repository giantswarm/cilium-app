...
| authentication.mutual.spire.install.initImage | object | `{"digest":"sha256:9db7b59979c38555a39def84a31fb98b5296952f9e3afd4f6f11f05b07adfab0","override":null,"pullPolicy":"IfNotPresent","repository":"docker.io/library/busybox","tag":"1.37.0","useDigest":true}` | init container image of SPIRE agent and server |
...
| certgen.tolerations | list | `[{"operator":"Exists"}]` | Node tolerations for pod assignment on nodes with taints ref: https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/  Giant Swarm tolerates any node startup taints here to avoid chicken-and-egg situations. For example, `NoExecute / ebs.csi.aws.com/agent-not-ready` is set for all nodes before the AWS EBS CSI driver HelmRelease is even installed, which depends on Cilium to be installed first. |
